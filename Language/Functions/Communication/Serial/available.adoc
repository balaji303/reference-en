...
void loop() {
  ...
  if (Serial01.available()) {             //Serial01 - Serial read from port 0, send to port 1
    int inByte = Serial01.read();
    Serial10.print(inByte, DEC);
  }
  // read from port 1, send to port 0:
  if (Serial10.available()) {             //Serial10 - Serial read from port 1, send to port 1
    int inByte = Serial10.read();
    Serial01.print(inByte, DEC);
  }
}
----
